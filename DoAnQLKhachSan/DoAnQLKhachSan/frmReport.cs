...
namespace DoAnQLKhachSan
{
    public partial class frmReport : Form
    {
        ...
        public frmReport(int ID,int ID_KH,string MaHD)
        {
            this.ID_HD = ID;
            this.ID_KH = ID_KH;
            this.MaHD = MaHD;
        ...
        {
            HienThiHD(this.ID_HD,this.ID_KH,this.MaHD);
            this.reportHD.RefreshReport();
     
        }
        private void HienThiHD(int ID,int ID_KH,string MaHD)
        {
            using (var _dbContext = new QLKhachSanConText())
            {
                string sql = "select MaHD,NgayDen,NgayDi,SoPhong,TongTienPhong,TongTienDV,TongTien from HoaDon,Phong where HoaDon.ID_Phong = Phong.ID and HoaDon.MaHD = " + "'"+ MaHD+"'";
